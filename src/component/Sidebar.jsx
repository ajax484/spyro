  ...
  return (
    <li className="">
      <h2>
        <button
          type="button"
          className={`flex items-center justify-between w-full py-4 px-5 font-medium text-left focus:border-l-primary focus:border-l-4 focus:bg-primary/10 rounded-sm ${
            route.mainpath === mainRoute ? "bg-primary/10" : ""
          }`}
          aria-expanded="true"
      ...
      >
        <ul ref={contentRef}>
          {route.children.map((subRoute) => (
            <li className={`py-1.5 ${currRoute === subRoute.path ? "" : ""}`}>
              <NavLink
                to={`${route.mainpath}/${subRoute.path}`}
                className={({ isActive }) =>
                  `${
